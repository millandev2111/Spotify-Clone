...
const SongControl = ({audio}) => {
  ...
  return(
    <div className="flex gap-x-3 text-xs p-1">
      <span className="opacity-50 text-right">{formatTime(currentTime)}</span>
        <Sliider
        defaultValue={[0]}
        value={[currentTime]}
        ...
        min={0}
        className="w-[400px]"
        onValueChange={(value) => { //revisar porque a el no le funciona y a mi si 
          // audio.current.currentTime = value (esta es mi manera de hacerlo)
